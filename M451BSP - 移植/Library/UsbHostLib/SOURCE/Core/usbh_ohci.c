 ...
 * @brief    M451 MCU USB Host OHCI driver
 *
 * @note
...
static int sohci_return_urb(URB_T * urb)
{
    ...
    {
        case PIPE_INTERRUPT:
            urb->complete(urb);    /* call complete and re-queue URB */
            urb->actual_length = 0;
        ...
        case PIPE_ISOCHRONOUS:
            ...
            if(urbt)
            {
                /* send the reply and re-queue URB */
                urb->complete(urb);
                urb->actual_length = 0;
                urb->status = USB_ERR_URB_PENDING;
...
/* add/re-init an endpoint; this should be done once at the usb_set_configuration command,
...
static void  td_fill(uint32_t info, void *data, int len, URB_T *urb, int index)
{
    ...
    if(index >= urb_priv->length)
    {
        USB_error("Error! - internal OHCI error: TD index [%d] > length [%d]\n", index, urb_priv->length);
        return;
    }
...
static void  dl_del_list(uint32_t frame)
{
    ...
    /* maybe re-enable control and bulk lists */
    if(!ohci.disabled)
    {
...
static uint8_t root_hub_config_des[] =
{
    ...
    0x40,       /*  uint8_t  bmAttributes;
                 Bit 7: Bus-powered, 6: Self-powered, 5 Remote-wakeup, 4..0: resvd */
...
#ifndef AUTO_POWER_CONTROL
    /* Over current status is not reported */
    USBH->HcRhDescriptorA |= RH_A_NOCP;
#endif

    /* global port power mode */
    USBH->HcRhDescriptorA = USBH->HcRhDescriptorA & ~(RH_A_PSM | RH_A_OCPM);
