...
int32_t main (void)
{
     ...
     *------------------------------------------------------------*/
//    if(FMC_Read(FMC_APROM_BASE)==0xFFFFFFFF)
//    {
//        printf("Don't find boot loader\n");
//        printf("Writing fmc_ld_boot.bin image to APROM...\n");
//        FMC_EnableAPUpdate();
//        if (load_image_to_flash((uint32_t)&loaderImage1Base, (uint32_t)&loaderImage1Limit,
//                                FMC_APROM_BASE, FMC_APROM_SIZE) != 0) {
...
//    }
		...
		while(1){
			...
			if((Qhuan=='y')||(Qhuan=='Y')){
				...
				BranchTo(FMC_APROM_BASE);
			}else if((Qhuan=='n')||(Qhuan=='N')){
				break;
			}
		}
    while(1)
    {
			ch=FMC_GetBootSource();
			if(ch){
				printf("\n\n 在LDROM56启动 \n");
			}else{
				printf("\n\n 在APROM56启动 \n");
